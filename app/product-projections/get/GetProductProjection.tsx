...
import { productProjectionGet } from './productProjectionGet';

const GetProductProjection = async () => {
  ...
  //   const response = (await productProjectionGet('id', paymentId))?.body;

  //   const paymentKey = 'sebastian-mueller-01';
  //   const keyResponse = (await productProjectionGet('key', paymentKey))?.body;
  const searchProductProjection = await productProjectionGet('search');
  const predicateProductProjection = await productProjectionGet('predicate');

  //   const allProductProjection = await productProjectionGet();
  //   const latestCreatedPayment = (await productProjectionGet())?.body?.results?.pop();

  //   const secondLastPayment = (await productProjectionGet())?.body?.results?.[
    ...
    <div className="grid grid-cols-2 gap-5 m-10 justify-center align-middle text-center">
      ...
      {/* <div className="ring-2 ring-blue-400 rounded-2xl col-span-2">
        <h2 className="font-extrabold text-2xl my-5 bg-blue-100">
          All Product Projections in this project key:{' '}
          {process.env.CTP_PROJECT_KEY}
        </h2>
        <h2>{JSON.stringify(allProductProjection)}</h2>
      </div> */}

      <div className="ring-2 ring-blue-400 rounded-2xl col-span-2">
        <h2 className="font-extrabold text-2xl my-5 bg-blue-100">
          Search Product Projections in this project key:{' '}
          {process.env.CTP_PROJECT_KEY}
        </h2>
        <h2>{JSON.stringify(searchProductProjection)}</h2>
      </div>
      <div className="ring-2 ring-blue-400 rounded-2xl col-span-2">
        <h2 className="font-extrabold text-2xl my-5 bg-blue-100">
          Predicate Product Projections in this project key:{' '}
          {process.env.CTP_PROJECT_KEY}
        </h2>
        <h2>{JSON.stringify(predicateProductProjection)}</h2>
      </div>
...
export default GetProductProjection;
